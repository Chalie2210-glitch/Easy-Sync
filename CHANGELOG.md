...
- 다른 Windows 11 x64 PC의 설치 경로에 맞춰 메뉴를 등록하고 COM 로딩을 검증합니다. 등록 실패는 설치 안내와 종료 코드에 반영합니다.
- 메뉴 복구 화면에 Windows 11의 추가 옵션 표시 위치를 안내하고, 설치 파일 빌드 시 외부 개발 런타임 의존성을 검사합니다.
- x64 메뉴 모듈을 로드할 수 없는 ARM64 OS에서는 이 설치 파일의 실행을 제한합니다.

- 대량 파일 선택 시 탐색기 메뉴가 숨겨지는 문제를 해결하기 위해 COM 명령으로 전환했습니다.
- 선택 경로를 임시 목록 파일로 한 번에 전달해 파일별 중복 실행과 명령줄 길이 제한을 피합니다.
- 오디오 외의 파일이 섞인 전체 선택에서도 메뉴를 표시하며 지원 오디오만 추가합니다.
- 메뉴 DLL은 내용별 파일 이름으로 설치하여 탐색기가 사용 중인 모듈을 덮어쓰지 않고 업데이트합니다.
